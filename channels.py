...
# récupère l'html d'une URL avec la librairie requests_html
...
# adapté aux pages Lyzem
# credit J. BAUMGARTNER/PUSHSHIFT
...
# adapté aux pages Telegago : méthode pour extraire les noms de canaux du .html
...
# effectuer une recherche et appliquer la méthode précédente sur chaque page de résultat
def search_channels_telegago(query, limit=100):
    initial_request_url = TELEGAGO_BASE_URL + urllib.parse.quote(query) + "&gsc.sort=date"
...
# Adapté au moteur de recherche TGSTAT (html récupéré manuellement)
