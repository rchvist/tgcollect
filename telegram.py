...
class SyncTelegramClient:
    ...
    def get_channel_users(self, channel, limit=1000):
        ...
        return participants
    def fetch_from_keyword(self, keyword, size=100, max_id=None, min_id=None):
        """Method to fetch messages from a keyword (global search)"""

        logger.debug("Fetching up to %d messages from keyword %s" % (size, keyword))
        params = [None, keyword, size]
        kwargs = {}
